...
private:
    ...
    bool merge() {
        if (num_of_blocks < 4 * base_now) return false;
    ...
    int check(int n) {
        if (size_of_blocks[n] < 2 * base_now) return-1;
...
public:
    ...
    iterator insert(iterator pos, const T &value) {
        ...
        int temp = 0;
        if(pos.num) temp = size_of_blocks[pos.num-1];
        int token = check(pos.num);
        if(token ==-1)return iterator(blocks, blocks_end, pos.num, temp_cur, size_of_blocks,pos.index);
        else if(token == 1&&pos.index>=base_now) return iterator(blocks, blocks_end, pos.num+1, temp_cur, size_of_blocks,pos.index-base_now);
        else if(token == 1&&pos.index<base_now)return iterator(blocks, blocks_end, pos.num, temp_cur, size_of_blocks,pos.index);
        else if(token == 2&&pos.index>=base_now/2) {
            if ((pos.num + 1) % 2 == 0)
                return iterator(blocks, blocks_end, (pos.num + 1) / 2, temp_cur, size_of_blocks,
                                pos.index - base_now / 2);
            if ((pos.num + 1) % 2 == 1)
                return iterator(blocks, blocks_end, (pos.num + 1) / 2, temp_cur, size_of_blocks, pos.index);
        }
        else if(token == 2&&pos.index<base_now/2) {
            if((pos.num)%2==0)return iterator(blocks, blocks_end, (pos.num)/2, temp_cur, size_of_blocks,pos.index);
            if((pos.num)%2==1)return iterator(blocks, blocks_end, (pos.num)/2, temp_cur, size_of_blocks,pos.index+temp);
        }
    }
    /**
